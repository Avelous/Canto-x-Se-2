...
contract BaseV1Pair {
  ...
  function _update(uint balance0, uint balance1, uint _reserve0, uint _reserve1, uint _totalSupply) internal {
    ...
    if (timeElapsed > periodSize) {
      observations.push(
        Observation(blockTimestamp, reserve0CumulativeLast, reserve1CumulativeLast, totalSupplyCumulativeLast)
      );
    }
    reserve0 = balance0;
    reserve1 = balance1;
  ...
  function sampleReserves(uint points, uint window) public view returns (uint[] memory, uint[] memory) {
    ...
    for (; i < lastIndex; i += window) {
      ...
      _reserves0[index] =
        (observations[nextIndex].reserve0Cumulative - observations[i].reserve0Cumulative) /
        timeElapsed;
      _reserves1[index] =
        (observations[nextIndex].reserve1Cumulative - observations[i].reserve1Cumulative) /
        timeElapsed;
  ...
  function sampleSupply(uint points, uint window) public view returns (uint[] memory) {
    ...
    for (; i < lastIndex; i += window) {
      ...
      timeElapsed = observations[nextIndex].timestamp - observations[i].timestamp;
      _totalSupply[index] =
        (observations[nextIndex].totalSupplyCumulative - observations[i].totalSupplyCumulative) /
        timeElapsed;
      index = index + 1;
    }
